...
void lnorm(double** vectors_matrix, int n, int vec_length){
    ...
    double** lnorm_calc = allocateMem(n, vec_length);
    lnorm_calc = lnorm_calc(d_matrix, n, vec_length);
...
double** lnorm_calc(double** d_matrix, int n, int vec_length){
...
int eigengap_heuristic(double** laplacian_matrix, int n, int vec_length){
...
struct eigens* jacobi_calc(double** vectors_matrix, int n, int vec_length){
    ...
    int* lar_arr = largest_indexes(vectors_matrix, n);
    lar_i = lar_arr[0];
    lar_j = lar_arr[1];

    //calculate c,s by theta and t
    int* cs_arr = get_c_and_s(vectors_matrix, lar_i, lar_j);
    c = cs_arr[0];
    s = cs_arr[1];
    ...
    A_to_A_tag(vectors_matrix, to_copy, n); //copy the values of to_copy to original array( A = A')

    rotations_number++;
    
    } while((off) > EPSILON && rotations_number <= 100);

    double** v_trans_matrix = matrix_Transpose(v_matrix, n, vec_length);
    
    eigens* eigensArray = (eigens*) calloc(n, sizeof(struct eigens));
    if (eigensArray == NULL)
        error_occurred();  
    for(i = 0; i < n; i++){
...
double** matrix_Transpose(double** mat, int n, int vec_length){
    double** trans_matrix = allocateMem(n, vec_length);
...
//
double** build_matrix_t_eigen(struct eigens* eigensArray, int n, int k, int vec_length){
    double** t_matrix_eigen = allocateMem(n, vec_length);
    if (t_matrix_eigen == NULL)
        error_occurred();
    double** u_matrix = allocateMem(n, vec_length);
    if (u_matrix == NULL)
        error_occurred();
...
int get_vec_length(FILE *temp_file){
    int vec_length = 0;
    int first = 0;
    double vector_item;
    char comma;
...
int get_vec_num(FILE *temp_file){
    int n = 0;
    double vector_item;
    char comma;
    while(fscanf(temp_file, "%lf%c",&vector_item,&comma) == 2){
